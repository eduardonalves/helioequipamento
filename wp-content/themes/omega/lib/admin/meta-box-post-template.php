 ...
 * post is saved.  This file is only used if the theme supports the 'omega-template-hierarchy' feature.
 */

/* Add the post template meta box on the 'add_meta_boxes' hook. */
add_action( 'add_meta_boxes', 'omega_meta_box_post_add_template', 10, 2 );
add_action( 'add_meta_boxes', 'omega_meta_box_post_remove_template', 10, 2 );

/* Save the post template meta box data on the 'save_post' hook. */
add_action( 'save_post', 'omega_meta_box_post_save_template', 10, 2 );
add_action( 'add_attachment', 'omega_meta_box_post_save_template' );
add_action( 'edit_attachment', 'omega_meta_box_post_save_template' );
 ...
 * @return void
 */
function omega_meta_box_post_add_template( $post_type, $post ) {

	/* Get the post templates. */
	$templates = omega_get_post_templates( $post_type );
	...
	if ( ( true === $post_type_object->public ) && ( current_user_can( 'edit_post_meta', $post->ID ) || current_user_can( 'add_post_meta', $post->ID ) || current_user_can( 'delete_post_meta', $post->ID ) ) )
		add_meta_box( 'omega-post-template', __( 'Template', 'omega' ), 'omega_meta_box_post_display_template', $post_type, 'side', 'default' );
}
 ...
 * @return void
 */ 
function omega_meta_box_post_remove_template( $post_type, $post ) {

	/* Removes meta box from pages since this is a built-in WordPress feature. */
	if ( 'page' == $post_type )
		remove_meta_box( 'omega-post-template', 'page', 'side' );
	...
	elseif ( function_exists( 'bbp_get_topic_post_type' ) && bbp_get_topic_post_type() == $post_type )
		remove_meta_box( 'omega-post-template', bbp_get_topic_post_type(), 'side' );
	...
	elseif ( function_exists( 'bbp_get_reply_post_type' ) && bbp_get_reply_post_type() == $post_type )
		remove_meta_box( 'omega-post-template', bbp_get_reply_post_type(), 'side' );
}
 ...
 * @return void
 */
function omega_meta_box_post_display_template( $object, $box ) {

	/* Get the post type object. */
	$post_type_object = get_post_type_object( $object->post_type );

	/* Get a list of available custom templates for the post type. */
	$templates = omega_get_post_templates( $object->post_type );

	wp_nonce_field( basename( __FILE__ ), 'omega-post-meta-box-template' ); ?>

	<p>
		<?php if ( 0 != count( $templates ) ) { ?>
			<select name="omega-post-template" id="omega-post-template" class="widefat">
				<option value=""></option>
 ...
 * @return void|int
 */
function omega_meta_box_post_save_template( $post_id, $post = '' ) {
	...
	if ( !isset( $_POST['omega-post-meta-box-template'] ) || !wp_verify_nonce( $_POST['omega-post-meta-box-template'], basename( __FILE__ ) ) )
		return $post_id;

	/* Return here if the template is not set. There's a chance it won't be if the post type doesn't have any templates. */
	if ( !isset( $_POST['omega-post-template'] ) )
		return $post_id;

	/* Get the posted meta value. */
	$new_meta_value = $_POST['omega-post-template'];
